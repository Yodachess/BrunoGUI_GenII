...
//                  └─ "Tableau Random64 de 781 éléments"
...
{   /*      Note : à partir de http://hgm.nubati.net/book_format.html
         
    Un fichier Polyglot (.bin) est une suite de “entries” (entrées de 16 octets).
    Chaque entrée correspond à :    👉 “Si la position est décrite par la clé X, alors un des coups jouables est Y, avec un poids Z.”

    Format exact d’une entrée (16 octets)
    1. Key (8 octets)
        C’est la clé Zobrist 64 bits de la position.
        Toutes les entrées qui ont la même clé correspondent à la même position.
        Donc si tu veux connaître les coups pour ta position, tu calcules sa clé → puis tu cherches toutes les entrées avec ce Key.
    2. Move (2 octets)
        Le coup encodé (aPartirDe/vers/promotion).
        Pas écrit en texte (e2e4), mais en nombres binaires (cases 0..63).
    3. Weight (2 octets)
        Poids statistique du coup.
        Plus le nomlectureBinairee est grand, plus le coup est favorisé.
        En pratique on tire aléatoirement un coup pondéré par ces poids.
    4. Apprentissage (4 octets)
        Champ prévu pour des fonctions d’apprentissage (rarement utilisé).
        Tu peux généralement l’ignorer.

    Exemple imaginaire :
    Supposons que la position initiale ait pour clé Polyglot :  0x463b96181691fc9c  (en fait, c'est le cas :-))
    Le fichier .bin pourrait contenir plusieurs entrées avec ce même Key :
                    Key	                Move    Weight      Apprentissage
                    463b96181691fc9c	e2e4	45      	0
                    463b96181691fc9c	d2d4	30      	0
                    463b96181691fc9c	c2c4	20      	0
                    463b96181691fc9c	g1f3	25      	0
    Ça veut dire :
    Dans la position initiale, le book propose ces 4 coups.
    Le coup e2e4 est le plus recommandé (poids 45).
    Donc en résumé :

    Un “entry” = un (clé, coup, poids)
    ...
    {   // Classe pour gérer la bibliothèque d'ouvertures Polyglot
        ...
        {   // Trouve toutes les entrées dans le fichier Polyglot correspondant à la clé donnée
            ...
            {   // Recherche binaire pour trouver une entrée avec la clé donnée
                long milieu = bas + (haut - bas) / 2;
                fs.Seek(milieu * 16, SeekOrigin.Begin);

                fs.Read(buffer, 0, 16);

                ulong clefcourante = BinaryPrimitives.ReadUInt64BigEndian(buffer);

                if (clefcourante < clefPolyglot)
                {   // On est trop bas, on monte
                    bas = milieu + 1;
                }
                else if (clefcourante > clefPolyglot)
                {   // On est trop haut, on descend
                    haut = milieu - 1;
                }
                else
                {   // Clé trouvée
            ...
            {   // On recule pour trouver le début des entrées avec la même clé
                ...
                pos--;  // On continue à reculer
            }

            fs.Seek(pos * 16, SeekOrigin.Begin);
            while (fs.Position < fs.Length)
            {   // On lit toutes les entrées avec la même clé
                int octetsLus = fs.Read(buffer, 0, 16);
                if (octetsLus < 16) break;

                ulong entryKey = LectureUInt64BigEndianFromBytes(buffer, 0);    // Lit la clé de l'entrée
                if (entryKey != clefPolyglot) yield break;  // On a fini les entrées avec cette clé
                ushort move = BinaryPrimitives.ReadUInt16BigEndian(buffer.AsSpan(8));
        ...
        // Nouvelles méthodes pour lire à partir d'un tableau d'octets
        public static ulong LectureUInt64BigEndianFromBytes(byte[] bytes, int offset)
        {   // Lit un UInt64 en big-endian à partir d'un tableau d'octets
            var octets = new byte[8];
            Array.Copy(bytes, offset, octets, 0, 8);
            Array.Reverse(octets);
            return BitConverter.ToUInt64(octets, 0);
        }
        public static uint LectureUInt32BigEndianFromBytes(byte[] bytes, int offset)
        {   // Lit un UInt32 en big-endian à partir d'un tableau d'octets
            var octets = new byte[4];
            Array.Copy(bytes, offset, octets, 0, 4);
            Array.Reverse(octets);
            return BitConverter.ToUInt32(octets, 0);
        }
        public static ushort LectureUInt16BigEndianFromBytes(byte[] bytes, int offset)
        {   // Lit un UInt16 en big-endian à partir d'un tableau d'octets
            var octets = new byte[2];
            Array.Copy(bytes, offset, octets, 0, 2);
            Array.Reverse(octets);
        ...
        public static string DecodeCoup(ushort coup)
        {
            ...
            /*
            dans la spécification Polyglot, le roque est encodé comme un déplacement du roi seul vers la case d'arrivée de la tour 
            (et non la case finale réelle du roi après roque). C'est pourquoi on a :
            Petit roque blanc : e1 → h1 (le roi "va" vers la case de la tour)
            Grand roque blanc : e1 → a1
            Petit roque noir : e8 → h8
            Grand roque noir : e8 → a8
            ...
            */
            ...
            string caseDepart = colonnes[aPartirDe & 7] + ((aPartirDe >> 3) + 1).ToString();
            string caseArrive = colonnes[vers & 7] + ((vers >> 3) + 1).ToString();

            string promoPiece = promo switch
            {
                1 => "n",
                2 => "b",
                3 => "r",
        ...
        {   // Calcule la clé Zobrist 64 bits pour une position donnée en FEN
            ...
            if (jetons.Length < 4)
                throw new ArgumentException("FEN invalide: " + fen);

            string echiquier = jetons[0];       // Description de l'échiquier
            char trait = jetons[1][0];          // Couleur au trait ('w' ou 'b')
            string droit_au_roque = jetons[2];  // Droits de roque
            string case_enpassant = jetons[3];  // Case en passant
            ...
            while (p < echiquier.Length)        // Parcourt la chaîne FEN
            {                                   // et remplit le tableau plateau
                char c = echiquier[p++];
                if (c == '/')
                {
                    r--;
                ...
                plateau[f++, r] = c;
            }   // Le tableau plateau est maintenant rempli

            const string nom_pieces = "pPnNbBrRqQkK";
            ulong clefPolyglot = 0;
            ...
            for (f = 0; f <= 7; f++)
            {
                for (r = 0; r <= 7; r++)
                {
                    ...
                    if (c != '\0' && c != '-')
                    {
                        int p_enc = nom_pieces.IndexOf(c);
                        if (p_enc >= 0)
                        {
                            clefPolyglot ^= RandomPiece[64 * p_enc + 8 * r + f];
                        }
                    }
                }
            }
            // Console.WriteLine($"Clef après pièces: {clefPolyglot:X16}");

            // Droits de roque
            foreach (char c in droit_au_roque)
            {
                switch (c)
                {
                    case '-': break;
                    case 'K': clefPolyglot ^= RandomRoque[0]; break;
                    case 'Q': clefPolyglot ^= RandomRoque[1]; break;
                    case 'k': clefPolyglot ^= RandomRoque[2]; break;
                    case 'q': clefPolyglot ^= RandomRoque[3]; break;
                }
            }
            // Console.WriteLine($"Clef après roques: {clefPolyglot:X16}");

            // En passant
            if (case_enpassant != "-" && case_enpassant.Length == 2)
            {
                f = case_enpassant[0] - 'a';
                if (trait == 'b')
                {
                    if ((f > 0 && plateau[f - 1, 3] == 'p') ||
                        (f < 7 && plateau[f + 1, 3] == 'p'))
                    {
                        clefPolyglot ^= RandomEnPassant[f];
                    }
                }
                else
                {
                    if ((f > 0 && plateau[f - 1, 4] == 'P') ||
                        (f < 7 && plateau[f + 1, 4] == 'P'))
                    {
                        clefPolyglot ^= RandomEnPassant[f];
                    }
            ...
            // Console.WriteLine($"Clef après en passant: {clefPolyglot:X16}");

            // Trait
            if (trait == 'w')
            {
                clefPolyglot ^= RandomTrait[0];
            }
            // Console.WriteLine($"clefPolyglot finale: {clefPolyglot:X16}");

            return clefPolyglot;
        }

        public static readonly ulong[] Random64 =
        [   // Liste de clés Zobrist pour les pièces et situations (781 éléments de 64 bits)
