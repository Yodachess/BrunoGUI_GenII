...
- Les événements statiques sont invoqués **sans test de nullité** : appeler la logique hors du formulaire (tests) exige d'y abonner des gestionnaires vides.
- `MoteurUci` lance le moteur comme `Process` avec stdin/stdout redirigés. `ProcOutputDataReceived` s'exécute sur un **thread non-UI** et déclenche `AfficheUci`, `AfficheDonneesBrutes`, `AfficheCoupMoteur` ; les gestionnaires côté formulaire doivent passer par `InvokeRequired`/`Invoke`. Les réponses `bestmove (none)` / `0000` sont traitées comme mat ou pat. Le nombre de variantes est centralisé dans `MoteurUci.NombreLignesPV` (initialisé depuis le `.ini`, modifié via `DefinitMultiPV`) : ne pas envoyer `MultiPV` en dur.
